/* Change Log.
    May 13, 2007 (Farhan Ahammed)
        - Modified the output to display the running time in between '?' characters.
          e.g. total running time: 1.00 seconds ?1.000?

          Made a similar modification to the output of the number of bb nodes (the
          value is surrounded by '&' characters)

          This allows for easy extraction of the two values.
*/
...
int main(int ac, char **av)
{
    ...
    if (masterfname) {
        ...
        if (ncount < 3) {
			...
			trt = CCutil_zeit () - szeit;
			printf ("Total Running Time: %.2f (seconds) ?%f?", trt, trt);
